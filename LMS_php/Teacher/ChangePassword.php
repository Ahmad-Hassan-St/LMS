...
?>
    ...
    $id = $_SESSION["cnic"];
    if (count($_POST) > 0) {
        $result = mysqli_query($connect, "SELECT * from teacher WHERE cnic='" . $id . "'");
        $row = mysqli_fetch_array($result);
        if ($_POST["currentPassword"] == $row[6]) {
            mysqli_query($connect, "UPDATE teacher set password='" . $_POST["newPassword"] . "' WHERE cnic='" . $id . "'");
                        ...
                        <main>
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    ...
                                    <form class="needs-validation" method="POST" action="ChangePassword.php" novalidate>
                                        <div class="row g-3">

                                            <div class="col-12">
                                                <label for="email" class="form-label">Old Password</label>
                                                <input type="password" name="currentPassword" class="form-control" id="email" required>
                                            </div>

                                            <div class="col-12">
                                                <label for="address" class="form-label">New Password</label>
                                                <input type="password" name="newPassword" class="form-control" id="address" required>
                                            </div>
